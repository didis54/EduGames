#-------------------------------------------------------------------------------
# Name:        math_1.py
# Purpose:     Jeu de calcue de delta.
#
# Author:      Yann
#
# Created:     29/04/2016
# Copyright:   © Copyright Yann
#-------------------------------------------------------------------------------
#!/usr/bin/env python
...
def calculSol(mode) :
    ...
    """mode : permet de différencier les cas à une solution de ceux a deux solutions"""
...
def stat(mode) :
    ...
    """mode : permet d'indiqué à la fonction les moments où elle doit soustraire un points au nombre de points """
    global points
...
def score():
    """Fonction comptant le nombre de points et les ajoutes au profil"""
    score = int(cfg.get(name, "math_score")) + points
    nbr_partie = int(cfg.get(name, "math_nbr_partie")) + 1
    cfg.set(name, "math_score", str(score))
    cfg.set(name, "math_nbr_partie", str(nbr_partie))
    cfg.write(open("profils.cfg", 'w'))

def calcul(mode,essais) :
    """Fonction vérifiant les solution de l'équation"""
    """mode : permet de fractionner la fonction en plusieurs parti, que l'on fait s'enchainé avec un ordre logique"""
    """essais : permet de vérifié si l'utilisateur a fait une erreur, et dans ce cas d'adapter le texte affiché"""
    ...
    Btnquitter = Button(Etat,text="Retour",command=quitter)
...
def bouton(reponse):
    ...
    """reponse : permet de différencier l'entrée du bouton oui de celle du bouton non"""
    Btnnon.place_forget()
    Btnoui.place_forget()
    correction = ""
    global a,b,c,x,y
...
def openframe(profilName) :
    ...
    """profilName : permet de garder en mémoire le nom de l'utilisateur et d'assuré la continuité de notre programme, essentiel pour le retour au menu principal qui utilise les données du profil"""

    global minijeu_1,Init,equation,question,Btnnon,Btnoui,largeur,hauteur,points,name
    name = profilName

    cfg.read("profils.cfg")
    if "math_score" in cfg.options(name) and "math_nbr_partie" in cfg.options(name):
        pass
    else:
        cfg.set(name, "math_nbr_partie", '0')
        cfg.set(name, "math_score", '0')
        cfg.write(open("profils.cfg", 'w'))
    ...
    info = Label (Init,text="Dans cet exercice: \n Les équations seront résolues dans R. \n Les solutions seront arondit au centième.", fg="red").place(x=0,y=175,height=85,width=largeur)
    ...
    Btnquitter = Button(Init,text="Retour",command=quitter)
    Btnquitter.place(x=0,y=0,height=25,width=75)
